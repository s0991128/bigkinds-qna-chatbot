...
async function main() {
  ...
  try {
    ...
    const scenarios = [
      ...
      { id: "S30", name: "historical lookup condition edit", run: async (p) => { await ask(p, "1997년 7월경 매일경제에 실린 아시아나 직원의 노동부 장관 표창 명단을 찾고 싶어요."); await p.clickByText("조건 수정", { exact: true }); await ask(p, "기간을 1997년 6~8월로 바꿔줘, 언론사는 매일경제만, 아시아나항공도 검색어에 넣어줘."); await p.waitFor("[...document.querySelectorAll('[data-qa=\\\"assistant-message\\\"]')].at(-1)?.getAttribute('data-qa-intent') === 'HISTORICAL_ARTICLE_LOOKUP'", 10000, "updated lookup intent"); const text = await p.latestAssistantText(); expectIncludes(text, "1997년 6~8월"); expectIncludes(text, "매일경제"); expectIncludes(text, "아시아나항공"); expectNotIncludes(text, "1990년부터"); await p.clickByText("이 조건으로 찾기", { exact: true }); await p.waitFor("document.querySelector('[data-qa=\\\"lookup-strategies\\\"]') !== null", 10000, "updated lookup strategies"); const strategies = await p.text('[data-qa="lookup-strategies"]'); expectIncludes(strategies, "1997-06-01"); expectIncludes(strategies, "1997-08-31"); expectIncludes(strategies, "매일경제"); } },
      { id: "S31", name: "historical search transfer payload", run: async (p) => { await ask(p, "1997년 7월 매일경제에 실린 아시아나 직원 자료를 찾고 싶어요"); await p.clickByText("이 조건으로 찾기", { exact: true }); await p.waitFor("document.querySelector('[data-qa=\\\"lookup-strategies\\\"]') !== null", 10000, "lookup strategies"); const expectedQuery = await p.evaluate("document.querySelector('[data-qa=\\\"lookup-strategies\\\"] code')?.textContent || ''"); assert.ok(expectedQuery, "lookup strategy query is missing"); await p.evaluate("(() => { window.__QA_SEARCH_PAYLOAD__ = ''; HTMLFormElement.prototype.submit = function () { window.__QA_SEARCH_PAYLOAD__ = this.querySelector('[name=\\\"jsonSearchParam\\\"]')?.value || ''; }; })()"); await p.clickByText("빅카인즈에서 검색", { exact: true }); await p.waitFor("window.__QA_SEARCH_PAYLOAD__ !== ''", 5000, "structured BIGKinds search payload"); const payload = await p.evaluate("JSON.parse(window.__QA_SEARCH_PAYLOAD__)"); assert.equal(payload.searchKey, expectedQuery); assert.equal(payload.startDate, "1997-07-01"); assert.equal(payload.endDate, "1997-07-31"); assert.deepEqual(payload.providerCodes, ["02100101"]); } },
      ...
      { id: "S32", name: "official Q&A snapshot metrics", run: async (p) => { await p.waitFor("document.querySelector('[data-qa=\"metric-qna\"] strong')?.textContent === '1,419'", 15000, "official Q&A count 1,419"); await p.waitFor("document.querySelector('[data-qa=\"metric-faq\"] strong')?.textContent === '23'", 15000, "official FAQ count 23"); await p.waitFor("document.querySelector('[data-qa=\"grounded-document-count\"] strong')?.textContent === '67'", 15000, "grounded document count 67"); } },
