...
export async function POST(request: Request) {
  let payload: { question?: unknown; sources?: unknown };
  try {
    const rawBody = await request.text();
    if (rawBody.length > MAX_BODY_LENGTH) return Response.json({ error: "요청이 너무 큽니다." }, { status: 413 });
    payload = JSON.parse(rawBody) as { question?: unknown; sources?: unknown };
  } catch {
  ...
  if (sensitivePattern.test(question)) return Response.json({ error: "민감한 질문은 AI 보완 대상에서 제외합니다." }, { status: 400 });
  if (isLikelyGeneralKnowledgeQuestion(question)) return Response.json({ error: "저장된 빅카인즈 문서 범위를 벗어난 질문입니다." }, { status: 422 });
  const provider = getProvider();
  if (provider === "none") return Response.json({ error: "AI 연동이 비활성화되어 있습니다." }, { status: 503 });
  if (provider === "gemini" && !process.env.GEMINI_API_KEY) {
    return Response.json({ error: "Gemini 연동이 비활성화되어 있습니다." }, { status: 503 });
  }
  ...
  try {
    if (provider === "gemini") {
      ...
      if (!geminiKey) return Response.json({ error: "Gemini 연동이 비활성화되어 있습니다." }, { status: 503 });
      ...
      if (!openAiKey) return Response.json({ error: "OpenAI 연동이 비활성화되어 있습니다." }, { status: 503 });
    ...
    return Response.json({ error: "AI 서비스에 연결할 수 없습니다." }, { status: 502 });
  }
  if (!upstream.ok) return Response.json({ error: "AI 답변 생성에 실패했습니다." }, { status: 502 });
  const answer = provider === "gemini"
    ? textFromGeminiResponse(await upstream.json())
    : textFromResponse(await upstream.json());
  if (!answer) return Response.json({ error: "AI 답변이 비어 있습니다." }, { status: 502 });
  return Response.json({ answer, usedAi: true, provider, sourceIds: sources.map((source) => source.id).filter(Boolean) });
}
