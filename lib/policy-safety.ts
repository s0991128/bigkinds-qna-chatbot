...
const policyRules: Array<{ kind: PolicyHandoffKind; label: string; pattern: RegExp; guidance: string }> = [
  ...
  },
  {
    kind: "AI_REUSE",
    label: "AI 재이용",
    pattern: /(?:ai|인공지능|rag|검색증강|모델|학습).{0,28}(?:재이용|재사용|학습|저장|가공|배포|서비스화|상업)|(?:재이용|재사용|학습|저장|가공|배포|서비스화|상업).{0,28}(?:ai|인공지능|rag|검색증강|모델)/i,
  ...
  },
  {
    ...
    label: "연구 목적 라이선스",
    pattern: /(?:연구|학술|논문|대학|연구자).{0,32}(?:라이선스|이용권|사용|원문|반출|제공|이용)|(?:라이선스|이용권).{0,32}(?:연구|학술|논문)/i,
    guidance: "연구 목적이라도 원문 제공 방식, 반출·보관 범위, 인용과 2차 활용 조건을 확인해야 합니다. 연구 목적과 필요한 자료 범위를 정리해 공식 담당자에게 문의해 주세요.",
  },
  {
    kind: "COMMERCIAL_USE",
    label: "상업적 이용",
    pattern: /상업적(?:으로)?\s*이용|영리\s*(?:이용|목적)|판매(?:용|를)?|수익(?:화|을)|서비스화|사업(?:용|적)/i,
